...
Description:    This program gets the number of candidates, candidates' last names and votes received.
It calculates the percent of total votes for each candidate, determines the winner, and prints the results.
...
static int getNumberOfCandidates();
inline string trim(const string& s);
void getCandidatesInfo(string candidatesNames[], int candidatesVotes[], int numCandidates);
void calculatePercentageVotes(const int candidatesVotes[], double candidatesVotesPercent[], const int votesSum, int numCandidates);
int getVotesSum(const int candidatesVotes[], int numCandidates);
vector<int> determineWinners(const int candidatesVotes[], int numCandidates);
void printResults(const string candidatesNames[], const int candidatesVotes[], const double candidatesVotesPercent[], const vector<int>& winners, const int votesSum, int numCandidates);

constexpr int MAX_NAME_LENGTH = 20; // Global constant for max length of last name

int main() {
    // Get number of candidates
    int numCandidates = getNumberOfCandidates();

    // Get candidates' names & votes
    string* candidatesNames = new string[numCandidates]; // Store names in a dynamic array
    int* candidatesVotes = new int[numCandidates]; // Store votes in a dynamic array
    getCandidatesInfo(candidatesNames, candidatesVotes, numCandidates);

    // Get sum of all votes
    int votesSum = getVotesSum(candidatesVotes, numCandidates);

    // Calculate percentage of the total votes received by each candidate
    double* candidatesVotesPercent = new double[numCandidates]; // Store votes percent in a dynamic array
    calculatePercentageVotes(candidatesVotes, candidatesVotesPercent, votesSum, numCandidates);

    // Determine winner(s)
    vector<int> winners = determineWinners(candidatesVotes, numCandidates);

    // Print results
    printResults(candidatesNames, candidatesVotes, candidatesVotesPercent, winners, votesSum, numCandidates);

    // Clean up memory
    delete[] candidatesNames;
    delete[] candidatesVotes;
    delete[] candidatesVotesPercent;
...
static int getNumberOfCandidates() {
    /*
    This function gets the number of candidates from the user.
    */
    int numCandidates;
    cout << "Enter the number of candidates: ";
    while (!(cin >> numCandidates) || numCandidates <= 0) {
        cin.clear();
...
void getCandidatesInfo(string candidatesNames[], int candidatesVotes[], int numCandidates) {
    ...
    */
    cout << "Please enter candidates' last names and votes for " << numCandidates << " candidates." << endl;
